...
class IFDEntry:
	...
	def __init__(self, fd, offsetStart):
		...
		if self.dataLength > 4:
			...
			print("Reading from", hex(offsetStart+self.offset))
			self.data   = unpack(">{:}B".format(self.dataLength), fd.read(self.dataLength))[0]
		else:
			self.offset = 0
			self.data   = unpack(">L", fd.read(4))[0]
		self.data = dataFromBytes(self.format, self.components, bytes(self.data))
		print(self.format, self.components, self.dataLength, hex(self.offset))
		print(len(self.data), type(self.data))
		print(str(self.data).strip())

		fd.seek(self.location+12)
		print(self.printStr.format(self.tag, self.name, self.data))
	...
	def __init__(self, marker, fd):
		...
		if self.name == b'Exif':
			...
			if self.offset != fd.tell() - self.location - 10: #location - start - 10 bytes to first M
				print("Incorect IFD offset in marker", self.name, "Exiting.")
				sys.exit(1)
			print("Offset to IFD:", self.offset)
			print("Offset reference byte:", hex(self.location+10))
			self.IFD = IFD(fd, self.location+10)
			#print EXIF data here
		fd.seek(self.next)
		print(self.printStr.format(self.location, self.marker, self.size))
...
def main(args):
	...
	with open(args[0], "rb") as fd:
		if unpack(">H", fd.read(2))[0] != 0xFFD8:
			...
			sys.exit(1)
		marker = fd.read(2)
		while marker != 0xFFD9: #0xFFD9 == EOF in JPEG
...
def unpackRational(components, bytestr):
	...
	while size <= components:
		...
		if len(string) >= 8:
			ret.append("{:s}{:s}".format(*unpack(">LL", string)))
		size += 8
	return ret
